### 34.1 reflection and refraction at a plane surface
 - when rays diverge from **object point** $P$ and are reflected/refracted, the directions of outgoing rays are as if they had diverged from an **image point** $P'$
	- If they actually converge at $P'$ it is a *real image*, if they only appear to diverge, it is a *virtual image*
- object distance $s$ and image distance $s'$:
$$s=-s'$$
- (for a plane mirror)
- sign rules:
	- **object distance** is positive if on the same side as incoming light
...
$$\frac{1}{s}+\frac{1}{s'}=\frac{2}{R}$$
		- if $\alpha$ is increased, however, this isn't accurate. this is why spherical mirrors form a "smeared" image rather than a precise point image (spherical aberration)

#### focal point and focal length
- if the object is very far from the mirror ($s=\infty$) the incoming rays are parallel:
$$\frac{1}{\infty}+\frac{1}{s'}=\frac{2}{R}\implies s'=\frac{R}{2}$$
- **focal point** is the point where parallel rays *converge after reflection from **convex** mirror* or where *diverge after reflection from **concave** mirror*
	- distance from vertex to focal point is the **focal length**:
$$f=\frac{R}{2}$$
		- for *spherical mirrors*
	- *in a concave mirror:*
		- the parallel rays incoming from $\infty$ reflect and converge to the **focal point** $F$
		- opposite situation: object placed at focal point, so the rays diverging from the focal point reflect to form parallel outgoing rays $\rightarrow$ image is at infinity
	- *in a convex mirror:*
...
- draw 4 principal rays
	1. **ray parallel to axis** after reflection:
		- concave: passes through $F$
		- convex: appears to come from virtual $F$
	2. **ray through/toward focal point $F$**
		- reflected parallel to the axis
	3. **ray along the radius**
		- through or away from center of curvature $C$ intersects the surface normally and is reflected back along original path
...
- **plane mirror**
	- object-image distances: $\dfrac{1}{s}+\dfrac{1}{s'}=0$
	- lateral magnification: $m=-\dfrac{s'}{s}$
- **spherical mirror**
	- object-image distances: $\dfrac{1}{s}+\dfrac{1}{s'}=\dfrac{2}{R}=\dfrac{1}{f}$
	- lateral magnification: $m=-\dfrac{s'}{s}$
- **plane refracting surface**
	- object-image distances: $\dfrac{n_a}{s}+\dfrac{n_b}{s'}=0$
	- lateral magnification: $m=-\dfrac{n_as'}{n_bs}$
- **spherical refracting surface**
	- object-image distances: $\dfrac{n_a}{s}+\dfrac{n_b}{s'}=\dfrac{n_b-n_a}{R}$
	- lateral magnification: $m=-\dfrac{n_as'}{n_bs}$
...
- draw the principal rays to locate the image
- **3 principal rays**
	- **ray parallel to axis** 
		- (converging) goes through lens and passes through $F_2$
...
- for a converging lens, as the object gets closer to the focal point, the image gets further away
	...
	- when object is inside focal point, the image is virtual and larger than the object (behind focal point) 

### 34.7 magnifier
$$M=\frac{\theta'}{\theta}=\frac{y/f}{y/25\text{ cm}}=\frac{25\text{ cm}}{f}$$
- 25 cm is the *near point* (closest distance a human eye can focus on)
